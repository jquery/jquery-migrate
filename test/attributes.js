...
test( "attrHooks[\"value\"]", function() {
	...
	expectWarning( "textarea.attr('value')", 1, function() {
		equal( jQuery( "#area1" ).attr( "value" ), "foobar",
			"textarea.attr('value') returns property." );
	} );

	expectWarning( "input.attr('value', val)", 0, function() {
		var el = jQuery( "#text1" ).attr( "value", "foo" );
		equal( el[ 0 ].getAttributeNode( "value" ).value, "foo",
			"input.attr('value', val) sets attribute." );
	...
	expectWarning( "button.attr(...)", 0, function() {
		...
		if ( majorMinorPatch[ 0 ] > 1 ||
		    ( majorMinorPatch[ 0 ] === "1" && majorMinorPatch[ 1 ] >= 9 )
		) {
			equal( button.attr( "value" ), undefined, "button.attr('value') returns attribute." );
			equal( button.attr( "value", "foo" ).attr( "value" ), "foo",
				"button.attr('value', val) sets attribute." );
		} else {
			ok( !button.attr( "value" ), "button.attr('value') is falsy." );
			ok( true, "Can't assert consistency of button.attr('value', val) on jQuery<1.9.0." );
		}
		equal( button.attr( "value", "bar" ).html(), "Button",
			"button.attr('value') doesn't affect contents" );
	} );

	expectWarning( "div.attr(...)", 2, function() {
		var div = jQuery( "#foo" );
		equal( div.attr( "value" ), undefined, "div.attr('value') returns attribute." );
		equal( div.attr( "value", "bar" ).attr( "value" ), "bar",
			"div.attr('value', val) sets attribute." );
...
test( "boolean attributes (boolHook)", function() {
	...
	expectWarning( ".attr(checked)", 1, function() {
		jQuery( "#check2" ).prop( "checked", true )
			.prop( "checked", false ).attr( "checked", true );
	...
	expectWarning( ".attr(selected)", 1, function() {
		jQuery( "#option3d" ).prop( "selected", true )
			.prop( "selected", false ).attr( "selected", true );
	...
	equal( document.getElementById( "check2" ).checked, true,
		"Set checked attribute with 'checked'" );
	equal( jQuery( "#check2" ).prop( "checked" ), true, "Set checked attribute" );
	equal( jQuery( "#check2" ).attr( "checked" ), "checked", "Set checked attribute" );
	...
	equal( $radios.attr( "checked" ), $radios[ 0 ].checked ? "checked" : undefined,
		"Known booleans do not fall back to attribute presence (#10278)" );
	...
	expectWarning( ".attr(HTML5)", 0, function() {
		...
		equal( $text.attr( "autofocus", false ).attr( "autofocus" ), undefined,
			"Setting autofocus attribute to false removes it" );
