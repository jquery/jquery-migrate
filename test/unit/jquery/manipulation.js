...
QUnit.test( "Improperly closed elements", function( assert ) {
	...
	expectMessage( assert, "Elements not self-closable nested wrong", 4, function() {
	...
	expectNoMessage( assert, "Script doesn't run in processed strings", function() {
	...
	expectNoMessage( assert, "Elements not self-closable but tolerable", function() {
		jQuery( "<div class=wonky />" );
		jQuery( "<p style='width: 2%' />" );
		jQuery( "<p />" ).append( "<span aria-label='hello' />" );
	} );

	expectNoMessage( assert, "Bare elements", function() {
